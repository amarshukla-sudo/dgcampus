...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════
        ...
        ib(
            "Upload your <b>Syllabus</b> to auto-fill topics and descriptions. "
            "Configure modules, TLOs, timing, and faculty ID. "
            "Preview and edit before generating the sheet in exact <b>DL.xlsx format</b>."
        )
        ...
        with b1:
            ...
            syl_file = st.file_uploader("",type=["docx","xlsx","xls","txt","csv"],
                                        key="syl_up",label_visibility="collapsed")
            if syl_file:
                st.markdown(f'<span class="badge">✅ {syl_file.name}</span>',
                            unsafe_allow_html=True)
        with b2:
            st.markdown("**📅 Master Attendance Sheet**  *(optional — for dates)*")
            st.caption("If provided, session dates are read from the attendance sheet")
            att_file_s = st.file_uploader("",type=["xlsx","xls"],key="att_s_up",
                                          label_visibility="collapsed")
            if att_file_s:
                st.markdown(f'<span class="badge">✅ {att_file_s.name}</span>',
                            unsafe_allow_html=True)

        syl_data = st.session_state.syl_data
        if syl_file and syl_file.name!=st.session_state._syl_nm:
            try:
                syl_data = parse_syllabus(syl_file.read(), syl_file.name)
                st.session_state.syl_data = syl_data
                st.session_state._syl_nm  = syl_file.name
                n_tp = sum(len(v) for v in syl_data.values())
                ok(f"Syllabus loaded — <b>{len(syl_data)} unit(s)</b> · <b>{n_tp} topics</b>")
            except Exception as ex:
                wn(f"Syllabus warning: {ex}")

        syl_data  = syl_data or OrderedDict()
        syl_units = list(syl_data.keys())

        # Load dates from attendance if uploaded
        sess_dates = []
        if att_file_s:
            try:
                att_s = read_master_attendance(att_file_s.read())
                sess_dates = att_s["dates"]
                ok(f"Dates loaded from attendance — {len(sess_dates)} session dates")
            except: pass
        ...
        with sc[0]:
            ...
            if sess_dates:
                use_custom = st.checkbox("Custom count", value=False)
                if use_custom:
                    total_sessions=st.number_input("Count",min_value=1,max_value=500,
                                                    value=len(sess_dates))
                else:
                    total_sessions=len(sess_dates)
                    st.metric("Sessions (= attendance dates)", total_sessions)
            else:
                total_sessions=st.number_input("Number of sessions",
        ...
        # Timing
        sec("Timing (per day)")
        ib("Enter Start and End time for each day that has classes. Format: HH:MM or 10:00 AM")

        detected={}
        for d in sess_dates: dn=d.strftime("%A"); detected[dn]=detected.get(dn,0)+1
        ...
        for day in ALL_DAYS:
            ...
            if enabled:
                with cs[2]: sv=st.text_input("",value="10:00",key=f"sv_{day}",
                                              label_visibility="collapsed")
                with cs[3]: ev=st.text_input("",value="11:00",key=f"ev_{day}",
                                              label_visibility="collapsed")
        ...
        # Units & Modules
        sec("Units & Module Configuration")
        unit_configs=[]

        if not syl_units:
            wn("No syllabus uploaded. Session titles will be auto-numbered.")
        else:
            ib(f"<b>{len(syl_units)} unit(s)</b> found. "
               "Choose how many to include and configure each module.")

            n_inc=st.number_input(f"How many units to include? (Syllabus has {len(syl_units)})",
                                   min_value=1,max_value=len(syl_units),value=len(syl_units))

            for i,uk in enumerate(syl_units[:n_inc]):
                avail=syl_data[uk]
                with st.expander(f"Unit {i+1} · {uk[:65]} ({len(avail)} topics)",
                                  expanded=(i<3)):
                    r1,r2=st.columns([3,2])
                    with r1:
                        mod_name=st.text_input(
                            "Module Name* (appears in session sheet)",
                            value=uk,key=f"mn_{i}")
                    with r2:
                    ...
                    if "Custom" in cov:
                        ...
                        final_topics=chosen
                        st.caption(f"✅ {len(final_topics)} / {len(avail)} selected")
                    else:
                        final_topics=avail
                        with st.expander(f"Preview {len(avail)} topics"):
                            for j,t in enumerate(avail,1): st.write(f"`{j}.` {t}")

                    unit_configs.append({"module_name":mod_name,"tlo":tlo_str,
                                         "topics":final_topics})
        ...
        # Preview
        sec("Preview & Edit")
        ib("Click Build Preview → topics auto-balanced, descriptions auto-generated. "
           "Edit any cell before generating.")

        if st.button("🔄  Build Preview", type="secondary", use_container_width=True):
            if not faculty_id.strip():
                er("Enter Faculty Registration ID above.")
            else:
                flat=(balance_topics(unit_configs,total_sessions) if unit_configs else
                      [{"module":"Module","tlo":f"{tlo_pfx}1","title":f"Session {i+1}"}
                ...
                for idx in range(total_sessions):
                    if idx<len(use_d):
                        d=use_d[idx]; dn=d.strftime("%A")
                        sh,sm,eh,em=day_timing.get(dn,(10,0,11,0))
                        sdt=fmt_dt(d,sh,sm); edt=fmt_dt(d,eh,em)
                    else:
                        sdt=edt="TBD"
                    sess=flat[idx] if idx<len(flat) else {
                        "module":"Extra","tlo":f"{tlo_pfx}1",
        ...
        if st.session_state.sess_preview is not None:
            edited=st.data_editor(
                ...
                column_config={
                    ...
                    "Mandatory*":       st.column_config.SelectboxColumn(
                                            "Mandatory*",options=["TRUE","FALSE"],width=110),
            ...
            if st.button("⚡  Generate session_sheet.xlsx",
                ...
                ok(f"Session sheet generated — <b>{len(rows_out)} rows</b> "
                   f"in exact DL.xlsx format ✅")

                st.download_button(
                    "📋  Download session_sheet.xlsx",
                    data=xlsx, file_name="session_sheet.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True
                )
